    ...
    private IDictionary<URI, Double> makePageRanks(IDictionary<URI, ISet<URI>> graph,
        ...
        for (int i = 0; i < limit; i++) {
            ...
            for (KVPair<URI, ISet<URI>> pair : graph) {
                ...
                if (!links.isEmpty()) {
                    for (URI uri : links) {
                        double rank = temp.get(uri) + (webpageRanks.get(pair.getKey()) * 
                        			decay / (double) links.size());
                        temp.put(uri, rank);
                    }
                } else {
                    for (KVPair<URI, Double> tempPair : temp) {
                        temp.put(tempPair.getKey(), tempPair.getValue() + decay * 
                        			(webpageRanks.get(pair.getKey()) / graph.size()));
                    }
